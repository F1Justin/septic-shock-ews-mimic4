...
\noindent
\textbf{Background.}
Whether heart-rate dynamics become progressively less flexible before haemodynamic collapse
is an active question in sepsis early-warning research.
In hourly electronic health record (EHR) data, rising lag-1 autocorrelation (AC1) is more
plausibly interpreted as \emph{physiological rigidity} than as a demonstration of
high-frequency critical slowing down.
...
Rolling 12-h variance (MAP) and HR AC1 were computed over a 48-h pre-event window after
...
In the primary conditional logistic model, late-centred HR AC1 remained associated with shock
(OR\,=\,1.60, 95\,\%\,CI\,1.15--2.22, $p=0.005$); sedative exposure showed the strongest
independent association (OR\,=\,4.70, 3.73--5.93, $p<0.001$).
After additional adjustment for concurrent mean MAP and HR, the HR AC1 term was no longer
...
Legault et al.\ \citeyearpar{legault2024} found variance-class indicators more predictive
than autocorrelation-based indices in a longitudinal hemodialysis cohort.
Chen et al.\ \citeyearpar{chen2012} originally formalised Dynamic Network Biomarker (DNB)
theory, which identifies tipping points via rising intra-module variance and correlation;
Liu et al.\ \citeyearpar{liu2019} extended this to a single-sample landscape-DNB framework.
...
(median AUC 178.0 vs.\ 125.0\,mmHg$\cdot$min; Wilcoxon rank-sum $p = 0.273$),
nor did the between-group comparison (shock vs.\ control in the late segment:
$p = 0.110$; no-vasopressor subgroup: $p = 0.055$; see Figure~S1 in the Supplementary Material).
...
within-shock-group time comparison ($p = 0.273$) nor the between-group comparison
($p = 0.110$) reached significance.
...
Khalil L, George SV, Brown KL, Ray S, Arridge S.
Transitions in intensive care: investigating critical slowing down post extubation.
\textit{PLoS ONE}. 2025;20(1):e0317211.

\bibitem[Legault et al., 2024]{legault2024}
Legault V, Pu Y, Weinans E, Cohen AA.
Application of early warning signs to physiological contexts: a comparison of
multivariate indices in patients on long-term hemodialysis.
\textit{Front Netw Physiol}. 2024;4:1299162.
...
Detecting early-warning signals for sudden deterioration of complex diseases
by dynamical network biomarkers.
\textit{Sci Rep}. 2012;2:342.
...
\textit{Natl Sci Rev}. 2019;6(4):775--785.

\bibitem[Rector et al., 2021]{rector2021}
Rector JL, Gijzel SMW, Van De Leemput IA, Van Meulen FB, Olde Rikkert MGM, Melis RJF.
Dynamical indicators of resilience from physiological time series in geriatric inpatients:
lessons learned.
\textit{Exp Gerontol}. 2021;149:111341.
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    Neither the within-shock-group late vs.\ early comparison ($p = 0.273$)
    nor the between-group comparison ($p = 0.110$) was significant at patient-pair level.
