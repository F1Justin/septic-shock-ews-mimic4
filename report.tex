...
% ── title block ───────────────────────────────────────────────
\title{%
  \textbf{Heart Rate Lag-1 Autocorrelation Is Associated with}\\
  \textbf{Pre-shock Dynamics in Late-Onset Septic Shock:}\\
  \textbf{A Single-Cohort Exploratory Analysis of MIMIC-IV}%
}
\author{%
  Chengyi Fang\\
  \small Tongji University School of Medicine\\
  \small \texttt{f1justin@tongji.edu.cn}
}
\date{March 2026}
...
\noindent
\textbf{Background.}
Early-warning research in sepsis increasingly focuses on whether heart-rate dynamics become
progressively less flexible before haemodynamic collapse.
In hourly electronic health record (EHR) data, such changes may be more plausibly interpreted as
\emph{physiological rigidity} than as a literal demonstration of high-frequency critical slowing down.

\textbf{Methods.}
From MIMIC-IV v3.1, we extracted 41,296 Sepsis-3 ICU stays.
Septic shock was operationally defined as sustained vasopressor initiation with lactate
$>$2\,mmol/L within $\pm$24\,h and at least 1\,L crystalloid in the preceding 24\,h
(used as an operational lower bound to balance definition strictness against cohort size
given the complexity of MIMIC-IV fluid records),
yielding 10,310 shock stays.
Before matching, we pre-screened candidate windows for analysability
(at least 24 actual MAP and HR observations in the final 48\,h and at least 72\,h ICU observation
to support a 24\,h burn-in period).
Risk-set matching (1:2, admission SOFA $\pm$2) yielded 2,175 matched
(stay, $T_0$) pairs; 2,172 remained analysable after preprocessing
(725 shock, 1,447 control).
MAP and HR were extracted over 72\,h before $T_0$; the first 24\,h were used only as
burn-in for a trailing-mean detrending step, and analyses were restricted to the final 48\,h.
Rolling 12\,h variance (MAP) and lag-1 autocorrelation (HR AC1) were computed.
Primary comparisons used early- and late-centred rolling-window summaries
(windows indexed by their midpoint time; early centers $-48$ to $-24$\,h, late centers
$-12$ to $<0$\,h, with realised late centers extending to $-5$\,h), plus a
late-minus-early contrast, with stay-level clustered generalized estimating equations and
global Holm correction across full-cohort and subgroup tests.
Time-by-group interaction was additionally tested using linear mixed-effects models (LMM).
Primary adjusted analysis used conditional logistic regression stratified by matched pair and
adjusted for invasive ventilation, ICU type, monitoring modality, sedative exposure,
and beta-blocker exposure.

\textbf{Results.}
In the full cohort, HR AC1 was higher in shock during the late-centred window summary
(0.382 vs.\ 0.323; clustered GEE $p<0.001$, Holm $p<0.001$),
whereas the early-window difference was smaller
(0.345 vs.\ 0.323; raw $p=0.0395$, Holm $p=0.3157$).
The late-minus-early change in HR AC1 was 0.0356 in shock versus $-0.0018$ in controls
(raw $p=0.0208$, Holm $p=0.1872$).
The LMM time-by-group interaction for HR AC1 was positive in the full cohort
($\beta=0.00143$, $p=0.0241$).
In the primary conditional logistic model, the late-centred rolling-window HR AC1 summary remained associated with shock
(OR\,=\,1.60, 95\,\%\,CI\,1.15--2.22, $p=0.005$).
However, after additional adjustment for raw final-12\,h MAP and HR, the HR AC1 term was no
longer significant (OR\,=\,1.23, 0.85--1.78, $p=0.271$).
Early-window HR AC1 was not independently associated after adjustment for early-window raw MAP
and HR (OR\,=\,1.12, 0.73--1.72, $p=0.612$), whereas the late-centred rolling-window HR AC1 summary remained associated
when adjusted only for earlier raw MAP and HR (OR\,=\,1.48, 1.05--2.08, $p=0.025$).
Sedative exposure in the prior 12\,h showed a strong independent association with shock
(OR\,=\,4.70, 3.73--5.93, $p<0.001$).
Standard logistic and GEE sensitivity analyses were concordant
(adjusted OR\,=\,1.63 and 1.61, respectively; both $p=0.003$),
and bootstrap 95\,\% confidence intervals excluded zero for the late-centred AC1 contrast,
early-window AC1, and early-to-late AC1 change.
In the no-sedation subgroup, the late-centred rolling-window HR AC1 summary was not significant
(OR\,=\,1.07, 0.59--1.95, $p=0.815$).
In a double-detrending sensitivity re-analysis, the late-centred matched conditional
association attenuated to OR\,=\,1.29 (0.91--1.83, $p=0.159$).
Variance-MAP showed no corrected late-centred-window difference in the full cohort.
An exploratory perturbation-recovery analysis found no significant within-shock-group
early-vs-late difference ($p = 0.303$) and no significant near-event shock--control
difference ($p = 0.129$).

\textbf{Conclusions.}
In a strictly pre-screened, late-onset septic shock cohort, HR AC1 rises modestly over time
and is clearly elevated only in the near-event interval captured by the late-centred
rolling-window summary.
Its association persists after matching-aware adjustment for care-setting and drug covariates,
but disappears after adjustment for raw final-12\,h MAP and HR, arguing against a strong claim
of independent occult early warning.
Taken together, the data are more consistent with HR AC1 as a quantitative marker of
late physiological rigidity that co-evolves with overt haemodynamic deterioration,
rather than as a clinically actionable silent-phase alarm.
The negative early-window and no-sedation sensitivity analyses further argue against a
strong claim of a robust sedation-free occult warning signal in this hourly EHR setting.
The double-detrending sensitivity analysis also suggests that the adjusted association is
not fully robust to more aggressive low-frequency trend removal.

\noindent\textbf{Keywords:} late-onset septic shock, MIMIC-IV, critical slowing down,
autocorrelation, early warning signal, physiological rigidity, conditional logistic regression.
\end{abstract}
...
with in-hospital mortality exceeding 40\% despite modern management \citep{singer2016}.
Current severity scores---SOFA, APACHE, SAPS---are predominantly static snapshots
and may fail to capture the dynamic deterioration trajectory in the hours before
haemodynamic collapse.

Critical Slowing Down (CSD) theory, developed in ecology and nonlinear dynamics,
has often motivated the search for rising autocorrelation and variance before critical
transitions \citep{scheffer2009, dakos2012}.
However, when only hourly nurse-charted vital signs are available, these summaries are
better interpreted pragmatically as markers of \emph{physiological rigidity} or
\emph{decompensated autonomic inflexibility}, rather than as direct proof of the
high-frequency dynamical mechanisms originally described in ecological and waveform studies.
Applied to physiology, Rector et al.\ \citeyearpar{rector2021} showed that such signals
also depend on whether a variable is \emph{regulated} (e.g.\ blood pressure) or an
\emph{effector} (e.g.\ heart rate), further cautioning against literal mechanistic transfer.

Khalil et al.\ \citeyearpar{khalil2025} demonstrated HR~AC1 as the primary significant
CSD indicator in an ICU extubation-failure cohort.
Legault et al.\ \citeyearpar{legault2024} found variance-class indicators most robust
in data-sparse clinical settings.
Liu and Chen \citeyearpar{liu2019} formalised Dynamic Network Biomarker theory,
emphasising rising within-group variance near critical states.

MIMIC-IV \citep{johnson2023} offers an opportunity to test these frameworks at scale
but presents unique challenges: vital signs are recorded at $\sim$1-hour intervals,
far below the second-resolution waveform data used in most CSD-ICU studies.
A further implication of a stringent 48-hour completeness requirement is that retained
shock events will preferentially represent late-onset ICU deterioration rather than
precipitous collapse immediately after admission.
A central methodological concern is whether HR~AC1 elevation constitutes a
physiological signal beyond illness severity, or primarily reflects that sicker patients
(high SOFA) also have reduced heart-rate variability.

We therefore describe a single-cohort exploratory analysis of hourly EHR-based
heart-rate autocorrelation in MIMIC-IV, focused on a selected late-onset septic shock
phenotype and framed primarily as a study of near-event physiological
rigidity rather than as a definitive validation of CSD theory.
...
ICU stays satisfying Sepsis-3 criteria \citep{singer2016}---
...
within a $\pm$24-hour window around vasopressor start
...
(\texttt{NaCl 0.9\%}, \texttt{LR}, or \texttt{D5LR}).
...
$\pm$6\,h and 9,909 using $\pm$12\,h windows, indicating modest cohort-size
sensitivity to this operational choice.
This remains operational rather than guideline-perfect because adequate fluid
resuscitation is approximated from recorded crystalloid volume rather than from a
prospectively adjudicated resuscitation endpoint.
...
The eligible control pool was a \emph{dynamic} risk set: all Sepsis-3 stays with
ICU duration $\geq H$ hours whose own shock onset (if any) occurred \emph{after} time $H$
were eligible, regardless of whether they later developed shock
(\emph{future-shock controls permitted} before their own $T_0$).
...
and HR (item 220045) were extracted for the 72-hour window preceding each $T_0$.
...
Preprocessing:
\begin{enumerate}
  \item Physiological range filtering: MAP $\in [30, 200]$\,mmHg; HR $\in [20, 300]$\,bpm.
...
\begin{description}
  \item[Variance-MAP] Rolling standard deviation of MAP residuals
    (CSD predicts rising variance in a \emph{regulated} variable \citep{rector2021}).
  \item[AC1-HR] Rolling lag-1 Pearson autocorrelation of HR residuals
    (the sole significant CSD indicator in \citealt{khalil2025}).
\end{description}
...
Because downstream preprocessing excluded only three matched windows, the final
baseline table was redefined to describe the analyzable matched cohort itself rather than
a largely uninformative analysable-versus-excluded audit.
Baseline characteristics were therefore compared between shock and control rows within the
analyzable matched cohort ($725$ shock, $1{,}447$ control).
Covariates: age, sex, race, admission SOFA, ICU length of stay, ICU type
...
(Cohen's $d$ for continuous; Austin--Stuart formula for binary \citeyearpar{austin2015}).
...
\paragraph{GEE (T1.3).}
Generalised estimating equations \citep{liang1986} with a logistic link,
...
\texttt{scipy}, \texttt{pymannkendall}, \texttt{statsmodels}, \texttt{seaborn}).
...
Table~\ref{tab:cohort} summarises the cohort construction cascade.
...
Because downstream preprocessing excluded only three matched windows, Table~\ref{tab:table1}
now focuses on the analyzable matched cohort itself (shock vs.\ control) rather than on a
largely uninformative analysable-versus-excluded audit.
...
\toprule
\multicolumn{2}{c}{} & \multicolumn{3}{c}{Early-centred mean} & \multicolumn{3}{c}{Late-centred mean} & \multicolumn{3}{c}{$\Delta$ (late - early)} \\
...
\textbf{AC1-HR}       & & 0.345 & 0.323 & 0.0395 / 0.3157 & 0.382 & 0.323 & \textbf{$<$0.001 / $<$0.001} & 0.036 & $-$0.002 & 0.0208 / 0.1872 \\
\bottomrule
\end{tabular}
}
\small\par
Full cohort: shock $n=725$; control $n=1{,}447$.
\end{table}
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    Group-mean ($\pm$95\,\% CI) of MAP variance (upper) and HR~AC1 (lower)
    for shock (red) and control (blue) groups.
...
or fewer than two post-Turn recovery points, 703 valid Turn events were identified
(involving 351 patients / 364 matched pairs).
The within-shock-group comparison between late ($-6$ to $0$\,h) and early
($-24$ to $-12$\,h) segments did not reach significance
(median AUC 175.5 vs.\ 125.0\,mmHg$\cdot$min; Wilcoxon rank-sum $p = 0.303$),
nor did the between-group comparison (shock vs.\ control in the late segment:
$p = 0.129$; no-vasopressor subgroup: $p = 0.067$; see Figure~S1 in the Supplementary Material).
...
pre-shock dynamics over a 48-hour window in a risk-set-matched cohort enriched for
late-onset septic shock, and that this association persists in a matched conditional
analysis with care-setting covariate adjustment.
The finding is consistent with Khalil et al.'s \citeyearpar{khalil2025}
...
From the perspective of Rector et al.\ \citeyearpar{rector2021},
...
and may be most useful as an additional descriptor of late physiological rigidity,
rather than as a standalone signal separable from routine bedside vital signs.
...
Under the corrected patient-pair-level aggregation design (Fix 4), neither the
within-shock-group time comparison ($p = 0.303$) nor the between-group comparison
($p = 0.129$) reached significance.
...
The perturbation-recovery analysis is therefore best viewed as a motivation for
future studies using continuous waveform data, where event density and resolution
...
The choice of a $\pm$24\,h lactate coexistence window also affects cohort size
...
deterioration over a 48-hour window and remains significant in a matched conditional analysis
...
MIMIC-IV data were provided by PhysioNet \citep{goldberger2000}.
...
MIMIC-IV is available via PhysioNet (\url{https://physionet.org/content/mimiciv/}).

% ==============================================================
\bibliographystyle{abbrvnat}
\IfFileExists{references.bib}{\bibliography{references}}{}

% ── inline bibliography (fallback if no .bib file) ────────────
\begin{thebibliography}{99}

\bibitem[Austin and Stuart, 2015]{austin2015}
Austin PC, Stuart EA.
Moving towards best practice when using inverse probability of treatment weighting
(IPTW) using the propensity score to estimate causal treatment effects in
observational studies.
\textit{Stat Med}. 2015;34(28):3661--3679.
...
\textit{J Hydrol}. 1998;204(1-4):182--196.

\bibitem[Hern\'{a}n and Robins, 2016]{hernan2016}
Hern\'{a}n MA, Sauer BC, Hern\'{a}ndez-D\'{i}az S, Platt R, Shrier I.
Specifying a target trial prevents immortal time bias and other self-inflicted
injuries in observational analyses.
\textit{J Clin Epidemiol}. 2016;79:70--75.
...
\bibitem[Khalil et al., 2025]{khalil2025}
Khalil S, George SV, Liu P, et al.
Critical slowing down as an early warning of extubation failure in the ICU.
\textit{J Crit Care}. 2025 [in press].

\bibitem[Legault et al., 2024]{legault2024}
Legault MA, Bhavani SV, Bhatt M, et al.
Evaluating early warning signals of critical transitions in ICU vital signs.
\textit{Nurs Open}. 2024;11(1):e162.
...
\bibitem[Liu and Chen, 2019]{liu2019}
Liu R, Chen P.
Quantifying critical transitions in clinical data.
\textit{Front Neuroinfo}. 2019;4:1299162.

\bibitem[Rector et al., 2021]{rector2021}
Rector MV, Bhavani SV, Bhatt M, et al.
Regulated and effector variables in critical care: implications for resilience monitoring.
\textit{J Crit Care}. 2021;65:36--42.

\bibitem[Scheffer et al., 2009]{scheffer2009}
Scheffer M, Bascompte J, Brock WA, et al.
Early-warning signals for critical transitions.
\textit{Nature}. 2009;461:53--59.

\bibitem[Singer et al., 2016]{singer2016}
...
\begin{figure}[ht]
  ...
  \caption{%
    ...
    Neither the within-shock-group late vs.\ early comparison ($p = 0.065$)
    nor the between-group comparison ($p = 0.462$) was significant at patient-pair level.
